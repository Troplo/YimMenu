...
namespace big
{
	    ...
	    m_error_packet_memmove_hook(g_pointers->m_gta.m_error_packet_memmove, hooks::error_packet_memmove)
	{
		if (strcmp(g_pointers->m_gta.m_online_version, "1.61") != 0)
		{
			...
			// Paragon: Required for -updateRpf and -update2Rpf arguments
			// detour_hook_helper::add<hooks::fipackfile_open_archive>("PARAGON_EXTRACONTENT_HOOK", g_pointers->m_gta.m_fipackfile_open_archive);
