...
namespace big::system
{
	...
	{
		DWORD64 base_address = memory::module(GetCurrentModule()).begin().as<DWORD64>();
		...
		for (auto& map : g_crossmap)
		{
			...
			file << std::hex << std::uppercase << "0x" << map.first << " : " << GetCurrentModule() << " + 0x" << (DWORD64)address - base_address << std::endl;
		}
