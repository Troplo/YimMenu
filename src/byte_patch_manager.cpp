...
namespace big
{
	static void init()
	{
		if (strcmp(g_pointers->m_gta.m_online_version, "1.61") != 0)
		{
			...
			// LOG(VERBOSE) << "Increase Start Get Presence Attributes limit from 32 to 100";
			...
			// LOG(VERBOSE) << "Prevent the attribute task from failing";
			...
			// LOG(VERBOSE) << "Prevent the game from crashing when flooded with outgoing events";
